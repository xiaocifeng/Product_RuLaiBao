package com.rulaibao.widget.superfile;
...
public class SuperFileView2 extends FrameLayout implements TbsReaderView.ReaderCallback {
    ...
    public void displayFile(File mFile,String filePath) {

        if (mFile != null && !TextUtils.isEmpty(mFile.toString())) {
            //增加下面一句解决没有TbsReaderTemp文件夹存在导致加载文件失败
//            String bsReaderTemp = "/storage/emulated/0/TbsReaderTemp";
            String bsReaderTemp = filePath+"/TbsReaderTemp";
            File bsReaderTempFile = new File(bsReaderTemp);

            if (!bsReaderTempFile.exists()) {
                Log.d(LOG_TAG, "准备创建/storage/emulated/0/TbsReaderTemp！！");
                boolean mkdir = bsReaderTempFile.mkdir();
                if (!mkdir) {
                    Log.e(LOG_TAG, "创建/storage/emulated/0/TbsReaderTemp失败！！！！！");
                }
            }
