...
import com.rulaibao.widget.webview.GetPathFromUri4kitkat;
import com.rulaibao.widget.webview.MyWebView;
import com.rulaibao.widget.webview.WebViewJSInterface;
import com.rulaibao.widget.webview.WebviewGlobals;
...
//				String fileName = FileUtils.getFileName(WebViewJSInterface.mCurrentPhotoPath);
				...
				if (myWebView.getMyWebChromeClient().getmUploadCallbackAboveL() != null) {
					...
					myWebView.getMyWebChromeClient().getmUploadMessage().onReceiveValue(uri);//将文件路径返回去，填充到input中
					myWebView.getMyWebChromeClient().setmUploadMessage(null);
				}
